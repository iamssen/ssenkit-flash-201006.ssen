...
package ssen.framework.widgets.input {
	 ...
	 */			
	public class TCM extends EventDispatcher implements ISWFContext, IInteractionEventHandler, ISandboxSupport {
		...
		static private const eventList : Array = [FlowOperationEvent.FLOW_OPERATION_BEGIN,
		...
		static tlf_internal const editingModePropertyDefinition : EnumStringProperty = new EnumStringProperty("editingMode", EditingMode.READ_WRITE, false, null, EditingMode.READ_WRITE, EditingMode.READ_ONLY, EditingMode.READ_SELECT);	
		 ...
		 * @langversion 3.0	 	
		 */
		public function TCM(container : Sprite,configuration : IConfiguration = null) {
		...
		public function compose() : void {
			if (_composeState == COMPOSE_COMPOSER)
				...
				if (_sourceState == SOURCE_TEXTFLOW && _textFlow.mustUseComposer())
					convertToTextFlowWithComposer(); else {
		...
		public function mouseOverHandler(event : MouseEvent) : void {
			if (_composeState == COMPOSE_COMPOSER)
				getController().mouseOverHandler(event); else {
...
import ssen.framework.widgets.input.TCM;
...
class TMContainerController extends ContainerController {
	private var _inputManager : TCM;

	public function TMContainerController(container : Sprite, compositionWidth : Number, compositionHeight : Number, tm : TCM) {
